        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" value={image} onChange={e => setImage(e.target.value)} required className="w-full bg-white dark:bg-brand-dark border border-gray-300 dark:border-brand-border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue" placeholder="https://example.com/image.jpg" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Excerpt</label>
              <textarea value={excerpt} onChange={e => setExcerpt(e.target.value)} required rows={5} className="w-full bg-white dark:bg-brand-dark border border-gray-300 dark:border-brand-border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue"></textarea>
            </div>
          </div>
        ...
        <div>
            ...
            <div className="bg-white dark:bg-brand-card border dark:border-brand-border rounded-lg shadow-sm overflow-hidden">
                ...
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                    {posts.map((post) => (
                        <tr key={post.title} className="bg-white dark:bg-brand-card border-b dark:border-brand-border">
                        ...
                        <td className="px-6 py-4 text-right whitespace-nowrap">
                            ...
                            <button onClick={() => handleDelete(post.title)} className="text-red-500 hover:text-red-700 p-2 ml-2">
